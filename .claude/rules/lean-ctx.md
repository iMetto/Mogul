...
**Write/Edit prerequisite:** Native `Write` and `Edit` require a prior native `Read` to confirm the file was seen.
A single `Read(path, limit=1)` immediately before `Write`/`Edit` is acceptable for this gate only — NOT for reading content.
All content reading must use `ctx_read`.
